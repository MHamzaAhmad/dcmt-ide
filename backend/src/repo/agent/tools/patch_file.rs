...
// Mirrored DTOs for patch_file (backend)
...
pub enum PatchPayload {
    ...
    DmpOps { dmp_ops: serde_json::Value },
}
...
impl PatchFileTool {
    ...
    let apply_range = |
        ...
        | -> Result<(), String> {
            ...
            if sl > el || (sl == el && sc > ec) { return Err("Invalid range".into()); }
            if sl == el {
                let line = &mut lines[sl];
                if sc > line.len() || ec > line.len() { return Err("Range exceeds line".into()); }
    ...
    struct Hunk { old_start: isize, old_len: isize, _new_start: isize, new_len: isize, lines: Vec<(char, String)> }
        ...
        for line in diff.lines() {
            if line.starts_with("@@") {
                ...
                let parse = |s: &str| -> Result<(isize,isize),String> {
                    let s = s.trim();
                ...
                if let Some(prefix) = line.chars().next() {
                    match prefix {
                        ' ' | '-' | '+' => h.lines.push((prefix, line[1..].to_string())),
                        '\\' => {}
                        _ => h.lines.push((' ', line.to_string())),
                    }
                }
            }
        }
            ...
            for (tag, content) in &h.lines {
                match *tag {
                    ' ' => {
                        ...
                        seg.push(exist.clone()); idx += 1; consume += 1;
                    }
                    '-' => {
    ...
    fn apply_dmp_ops(text: &str, dmp_ops: &serde_json::Value) -> Result<String, String> {
        ...
        for item in ops {
            ...
            match op_code {
                0 => { if !src.starts_with(&seg) { return Err("dmp equal segment does not match source".into()); } out.push_str(&seg); src = &src[seg.len()..]; }
                -1 => { if !src.starts_with(&seg) { return Err("dmp delete segment does not match source".into()); } src = &src[seg.len()..]; }
                1 => { out.push_str(&seg); }
...
impl super::AgentTool for PatchFileTool {
    fn name(&self) -> &str { "patch_file" }
    fn display_name(&self) -> &str { "Patch File" }
    fn progressive_form(&self) -> &str { "Patching file" }
    fn definition(&self) -> ToolDefinition {
        ToolDefinition { tool_type: "function".into(), function: FunctionDefinition {
            name: self.name().into(),
            description: "Patch a file with minimal edits using structured operations.".into(),
            parameters: serde_json::json!({
                ...
                "properties": {
                    ...
                    "patch": { "oneOf": [
                        ...
                        { "type": "object", "properties": { "dmp_ops": { "type": "array", "items": { "type": "object", "properties": { "op": {"type":"number"}, "text": {"type":"string"} }, "required": ["op","text"] } } }, "required": ["dmp_ops"] }
                    ]},
                    "precondition": { "type": "object", "properties": { "mtime": { "type": "number" }, "on_mismatch": { "type": "string", "enum": ["fail", "rebase"] } } }
                },
                "required": ["path", "patch"]
            }),
    ...
    async fn execute(&self, workspace: &Path, args: serde_json::Value, repo: Option<&crate::repo::agent::AgentRepo>) -> AgentResult<String> {
        let path = args["path"].as_str().ok_or_else(|| AgentError::InvalidToolArguments { tool: self.name().into(), error: "Missing 'path'".into() })?;
        if path.starts_with('/') || path.contains("..") { return Err(AgentError::InvalidToolArguments { tool: self.name().into(), error: "Invalid path".into() }); }
        let full_path = workspace.join(path);

        let existed = full_path.exists();
        let mut current = String::new();
        if existed { current = tokio::fs::read_to_string(&full_path).await.map_err(AgentError::IoError)?; }

        if let Some(pre) = args.get("precondition") { if let Some(mtime) = pre.get("mtime").and_then(|v| v.as_u64()) {
            if let Ok(meta) = tokio::fs::metadata(&full_path).await { if let Ok(modified) = meta.modified() {
                let ts = modified.duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis() as u64;
                if ts != mtime { return Err(AgentError::InvalidToolArguments { tool: self.name().into(), error: "Precondition mismatch (mtime)".into() }); }
            }}
        }}

        let patch_payload: PatchPayload = serde_json::from_value(args.get("patch").cloned().ok_or_else(|| AgentError::InvalidToolArguments { tool: self.name().into(), error: "Missing 'patch'".into() })?)
            .map_err(|e| AgentError::InvalidToolArguments { tool: self.name().into(), error: e.to_string() })?;

        let updated = match patch_payload {
            PatchPayload::Ops { ops } => Self::apply_ops(&current, &ops).map_err(|e| AgentError::InvalidToolArguments { tool: self.name().into(), error: e })?,
            PatchPayload::UnifiedDiff { unified_diff } => Self::apply_unified_diff(&current, &unified_diff).map_err(|e| AgentError::InvalidToolArguments { tool: self.name().into(), error: e })?,
            PatchPayload::DmpOps { dmp_ops } => Self::apply_dmp_ops(&current, &dmp_ops).map_err(|e| AgentError::InvalidToolArguments { tool: self.name().into(), error: e })?,
        };

        let repo = repo.ok_or_else(|| AgentError::InvalidToolArguments { tool: self.name().into(), error: "Repository access required".into() })?;
        let file_service = repo.get_file_service();
        file_service.write_file(path, &updated).await.map_err(|e| AgentError::IoError(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
