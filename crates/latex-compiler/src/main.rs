...
struct Args {
    ...
    #[arg(long, default_value = "/root/.TinyTeX/bin/x86_64-linux")]
    latex_bin_path: PathBuf,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("🔧 LaTeX binaries path: {}", args.latex_bin_path.display());

    // Add LaTeX binaries to PATH if specified
    if args.latex_bin_path.exists() {
        let current_path = std::env::var("PATH").unwrap_or_default();
        let new_path = format!("{}:{}", args.latex_bin_path.display(), current_path);
        std::env::set_var("PATH", new_path);
        info!("✅ Added LaTeX binaries to PATH");
    }
...
async fn verify_latex_installation() -> anyhow::Result<Vec<Engine>> {
    ...
    if available_engines.is_empty() {
        anyhow::bail!("No LaTeX engines found. Please install LaTeX (TinyTeX, TeX Live, or MiKTeX)");
    }
